...
from dalle2_pytorch.dalle2_pytorch import Decoder, DiffusionPrior, DiffusionPriorNetwork, OpenAIClipAdapter
...
def load_diffusion_model(dprior_path, device):
    ...
    assert dprior_path.exists(), f'Diffusion model file {dprior_path} does not exist'
    ...
    # Create CLIP (optional)
    clip_argparsed = dp_config.get('clip')
    if clip_argparsed == 'openai_clip':
        print('Using OpenAI CLIP-Adapter, no need load from ckpt')
        dp_config['clip'] = OpenAIClipAdapter()
...
class DecoderTrainer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.num_unes = len(self.decoder.unets)
        
        self.use_ema = use_ema
        
        if use_ema:
            has_lazy_linear = any([type(u) == nn.LazyLinear for u in decoder.modules()])
        ...
        lr, wd = map(partial(cast_tuple, length = self.num_unes), (lr, wd))
    ...
    def scale(self, loss, *, unet_number):
        assert 1 <= unet_number <= self.num_unes
        index = unet_number - 1
    ...
    def sample(self, *args, **kwargs):
        if self.use_ema:
            ...
            self.decoder.unets = self.unets # temporarily switch to the ema unets
            
        output = self.decoder.sample(*args, **kwargs)
        
        if self.use_ema:
            self.decoder.unets = trainable_unets # switch back to the trainable unets
