...
# keyword arguments helpers

def pick_and_pop(keys, d):
    values = list(map(lambda k: d.pop(k), keys))
...
def group_dict_by_key(cond, d):
    ...
    for key in d:
        match = bool(cond(key))
        ind = 0 if match else 1
        return_val[ind][key] = d[key]
    return (*return_val,)

def string_begin_with(prefix, str):
    return str.startswith(prefix)

def group_by_key_prefix(prefix, d):
    return group_dict_by_key(lambda k: string_begin_with(prefix, k), d)

def groupby_prefix_and_trim(prefix, d):
    kwargs_with_prefix, kwargs = group_dict_by_key(partial(string_begin_with, prefix), d)
...
def gradient_penalty(images, output, weight = 10):
    ...
    gradients = torch_grad(outputs=output, inputs=images,
                           grad_outputs=torch.ones(output.size()).to(images.device),
...
def hinge_disc_loss(fake, real):
    return (F.relu(1.0 - real) + F.relu(1.0 + fake)).mean()
...
class ContinuousPositionBias(nn.Module):
    ...
    def forward(self, x):
        ...
        if not exists(self.rel_pos):
            ...
            rel_pos = torch.sign(rel_pos) * torch.log(torch.abs(rel_pos) + 1)
...
class ResNetEncDec(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert dim % resnet_groups == 0, f'dimension {dim} is not divisible by {resnet_groups} (groups for the groupnorm)'
        ...
        assert len(layer_mults) == layers, f'layer_mults must have {layers} elements, got {len(layer_mults)}'
        ...
        assert len(num_resnet_blocks) == layers, f'num_resnet_blocks must have {layers} elements, got {len(num_resnet_blocks)}'
        assert len(use_attn) == layers, f'use_attn must have {layers} elements, got {len(use_attn)}'
        
        for layer_index, (dim_in, dim_out), layer_num_resnet_blocks, layer_use_attn in zip(range(layers), dim_pairs, num_resnet_blocks, use_attn):
            append(self.encoders, nn.Sequential(nn.Conv2d(dim_in, dim_out, 4, stride=2, padding=1), leaky_relu()))
            prepend(self.decoders, nn.Sequential(nn.ConvTranspose2d(dim_out, dim_in, 4, stride=2, padding=1), leaky_relu()))
...
class GLUResBlock(nn.Module):
    def __init__(self, chan, groups=16) -> None:
        super().__init__()
...
class VQGanVAE(nn.Module):
    ...
    def forward(
        ...
        assert height == self.image_size and width == self.image_size, f'height and width must be equal to image_size {self.image_size}'
        assert channels == self.channels, f'channels must be equal to the channels {self.channels} set on this VQGanVAE'
        ...
        assert return_loss ^ return_discr_loss, 'either return_loss or return_discr_loss, but not both'
        
        # whether to return discriminator loss
        
        if return_discr_loss:
            assert exists(self.discr), 'discriminator must be exists if return_discr_loss is True'
            fmap.detach_()
            img.requires_grad_()
            
            fmap_discr_logits, img_disc_logits = map(self.discr, (fmap, img))
            
            discr_loss = self.discr_loss(fmap_discr_logits, img_disc_logits)
            
            if add_gradient_penalty:
                gp = gradient_penalty(img, img_disc_logits)
                loss = discr_loss + gp
        ...
        if img.shape[1] == 1:
            # handle grayscale images for vgg
