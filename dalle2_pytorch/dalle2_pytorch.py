...
class BaseClipAdapter(nn.Module):
    def __init__(self, clip:Optional[CLIP], **kwargs) -> None:
        super().__init__()
        self.clip = clip
...
class OpenAIClipAdapter(BaseClipAdapter):
    def __init__(self, name='ViT-B/32') -> None:
        import clip
...
def discretized_gaussian_log_likelihood(x, *, means, log_sacles, thres=0.999):
    assert x.shape == means.shape == log_sacles.shape
    
    centered_x = x - means
    inv_stdv = torch.exp(-log_sacles)
    plus_in = inv_stdv * (centered_x + 1.0 / 255.)
    cdf_plus = approx_standard_normal_cdf(plus_in)
    min_in = inv_stdv * (centered_x - 1.0 / 255.)
    cdf_min = approx_standard_normal_cdf(min_in)
    log_cdf_plus = log(cdf_plus)
    log_one_minus_cdf_min = log(1.0 - cdf_min)
    cdf_dela = cdf_plus - cdf_min
    
    log_probs = torch.where(x<-thres,
                            log_cdf_plus,
                            torch.where(x>thres, log_one_minus_cdf_min, log(cdf_dela)))
    return log_probs
...
class ChanLayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.g * (x - mean) / torch.sqrt(var)
...
class MLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        norm_fn = lambda: nn.LayerNorm(dim_in) if norm else nn.Identity()
...
class DiffusionPriorNetwork(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # whether text embedding is masked or not depends on the classifier free guidance conditional maskimg
...
class DiffusionPrior(BaseGaussianDiffusion):
    def __init__(
        ...
        *,
        ...
        if exists(clip):
            ...
            assert exists(image_embed_dim), "latent dimension must be specified if clip is not specified"
            self.clip = None
    ...
    def p_sample(self, x, t, text_cond=None, clip_denoised=True, repeat_noise=False):
        ...
        model_mean, _, model_log_variance = self.p_mean_variance(x, t, text_cond, clip_denoised)
        noise = noise_like(x.shape, device=device, repeat=repeat_noise)
        # no noise when t==0
        nonzero_mask = (1 - (t==0).float()).reshape(b, *((1,)*len(x.shape[1:])))
        return model_mean + nonzero_mask * noise * torch.exp(model_log_variance / 2)
    ...
    def sample_batch_size(self, batch_size, text_cond):
        ...
        for i in tqdm(reversed(range(self.num_timesteps)), desc="sampling loop time step", total=self.num_timesteps):
            img = self.p_sample(img, t=torch.full((batch_size,), i, device=device, dtype=torch.long), text_cond=text_cond)
    ...
    def sample(self, text, num_samples_per_batch=2):
        ...
        # retrieve original unscaled image embed (p_sample_loop added a scale)
        image_embeds = image_embeds / self.image_embed_scale
        ...
        top_image_embeds = image_embeds.gather(dim=1, index=top_sim_indices)
    ...
    def forward(
        ...
        text_mask = None,   # text_mask <- may eventually opt for the learned padding tokens technique from DALL-E1 to reduce complexity
        *args,
        **kwargs
    ):
        assert exists(text) ^ exists(text_embed), f'either text or text_embed must be provided, not both'
        assert exists(image) ^ exists(image_embed), f'either image or image_embed must be provided, not both'
        assert not (self.condition_on_text_encodings and (not exists(text_encodings) and not exists(text))), f'if condition_on_text_encodings is True, text_encodings must be provided(at least provide text)'
        ...
        if self.condition_on_text_encodings:
            assert exists(text_encodings), f'if condition_on_text_encodings is True, text_encodings must be provided'
            text_cond = {**text_cond, 'text_encodings': text_encodings, 'mask': text_mask}
        
        # timestep conditioning from ddpm
        batch, device = image_embed.shape[0], image_embed.device
        times = torch.randint(low=0, high=self.num_timesteps, size=(batch,), device=device, dtype=torch.long)
        
        # scale image embed (Katherine)

        image_embed *= self.image_embed_scale
        
        # caluclate forward loss
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, 
                 *, cond_dim=None, time_cond_dim=None, groups=8):
        super().__init__()
    ...
    def forward(self, x, cond = None, time_emb = None):
        ...
        if exists(self.cross_attn):
            assert exists(cond), f'if cross_attn is True, cond must be provided'
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context, mask = None):
        ...
        # add null key and value for classifier free guidance in prior net
        nk, nv = repeat_many(self.null_kv.unbind(dim=-2), 'd -> b h 1 d', b = b, h = self.heads)
...
class GridAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.attn(x)
        out = rearrange(x, '(b h w) (w1 w2) c -> b c (w1 h) (w2 w)', w1 = wsz, w2 = wsz, h = h // wsz, w = w // wsz)
        return out
...
class LinearAttention(nn.Module):
    ...
    def forward(self, fmap):
        ...
        out = rearrange(out, '(b h) (x y) c -> b (h c) x y', h = h, x = x, y = y)
...
class CrossEmbedLayer(nn.Module):
    def __init__(
        ...
        # calculate the dimension at each sacle
        dim_scales = [int(dim_out / (2 ** i)) for i in range(1, num_scales)]
        dim_scales = [*dim_scales, dim_out - sum(dim_scales)]
        
        self.convs = nn.ModuleList([])
        for kernel_size, dim_scale in zip(kernel_sizes, dim_scales):
            self.convs.append(nn.Conv2d(dim_in, dim_scale, kernel_size, stride=stride, padding=(kernel_size - stride) // 2))
...
class Unet(nn.Module):
    def __init__(
        ...
        *,
        ...
        resnet_groups = 8,
        init_cross_kernel_kernel_sizes = (3, 7, 15),
        ...
        **kwargs
        ...
        self.init_conv = CrossEmbedLayer(init_channels, dim_out=init_dim, kernel_sizes=init_cross_kernel_kernel_sizes, stride=1)
        
        dims = [init_dim, *map(lambda m: m*dim, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))
        
        # time, image embeddings, and optional text embeddings
        # time embeddings
        ...
        self.to_time_tokens = nn.Sequential(
            nn.Linear(time_cond_dim, num_time_tokens * cond_dim),
        ...
        # finer control over whether condition on image embeddings and text encodings
        # so one can have the latter unets in the cascading DDPMs only focus on super-resolution
    ...
    def cast_model_parameters(
        ...
        *,
        ...
        channels,
        channels_out,
        cond_on_image_embeds:bool,
        cond_on_text_encodings:bool,
    ):
    ...
    def forward_with_cond_scale(
        ...
        **kwargs
        ...
        null_logits = self.forward(*args, text_cond_drop_prob=1., image_cond_drop_prob=0.1, **kwargs)
        return null_logits + (logits - null_logits) * cond_scale
    ...
    def forward(
        ...
        *,
        ...
        assert not (self.lowres_cond and not exists(lowres_cond_img)), f'lowres_cond_img is None, but lowres_cond is True'
        ...
        t = self.to_time_cond(time_hiddens) # (b, r, cond_dim), r:num_time_tokens
        
        # conditional dropout
        image_keep_mask = prob_mask_like((batch_size,), prob = 1. - image_cond_drop_prob, device = device)
        text_keep_mask = prob_mask_like((batch_size,), prob = 1. - text_cond_drop_prob, device = device)
        ...
        if self.cond_on_image_embeds:
            image_tokens = self.image_to_cond(image_embed) # (b, r, cond_dim), r:num_image_tokens
            
            null_image_embed = self.null_image_embed.to(image_tokens.dtype)
            image_tokens = torch.where(
                image_keep_mask,
                image_tokens,
                null_image_embed
            ) # dropout on the batch-dimension, not dropout sequence-elements
        ...
        if exists(text_encodings) and self.cond_on_text_encodings:
            text_tokens = self.text_to_cond(text_encodings) # (b, n, cond_dim), n:num_text_tokens
            ...
            if remainder > 0:
                text_tokens = F.pad(text_tokens, (0, 0, 0, remainder)) # pad at the end of sequence
            
            if exists(text_mask):
                if remainder > 0:
                    text_mask = F.pad(text_mask, (0, remainder), value = False) # mask: keep-mask, covering valid text-tokens
...
class Decoder(BaseGaussianDiffusion):
    def __init__(
        ...
        *,
        ...
        self.un_conditional = unconditional
        ...
        # whether to use learned variance, defaults to True for the first unet in the cascades
        ...
        self.sample_channels = cast_tuple(self.channels, len(self.image_sizes))
        
        # random crop sizes (for super resoluting unets at the end of the cascade)
        
        self.random_crop_sizes = cast_tuple(random_crop_sizes, len(self.image_sizes))
        
        # predict x0 config
        
        self.predict_x_start = cast_tuple(predict_x_start, len(unets)) if not predict_x_start_for_latent_diffusion else tuple(map(lambda t: isinstance(t, VQGanVAE), vaes)) # if predicting x0 for latent diffusion, we need to condition on the VAEs(ensure that the VAEs are not None/Null); otherwise, all unets are required to predict x0

        # cascading ddpm related stuff
        
        lowres_conditions = tuple(map(lambda t:t.lowres_cond, self.unets))
        assert lowres_conditions == (False, *((True,) * (len(self.unets) - 1))), 'you must provide unconditional setting on the first u-net, and the rest must be conditioned on the lowres condition of the previous u-net'
    ...
    def one_unet_in_gpu(self, unet_number = None, unet = None):
        assert exists(unet_number) ^ exists(unet), 'must provide either unet_number or unet'
    ...
    def p_mean_variance(self, unet:Unet, 
                        x, t, image_embed, text_encodings = None, text_mask = None,
                        lowres_cond_img = None, clip_denoised = True, predict_x_start = False,
                        cond_scale = 1.0, learned_variance=False, model_output=None):
        pred = default(model_output, 
                       lambda: unet.forward_with_cond_scale(x, t, image_embed = image_embed, text_encodings = text_encodings, text_mask = text_mask, lowres_cond_img = lowres_cond_img, cond_scale = cond_scale)
                       )
        ...
        if predict_x_start:
            ...
            x_recon = self.predict_start_from_noise(x, t, noise=pred)
    ...
    @torch.inference_mode()
    def p_sample(self, unet:Unet,
                x, t, image_embed, text_encodings = None, text_mask = None,
                lowres_cond_img = None, clip_denoised = True, predict_x_start = False,
                learned_variance = False, cond_scale = 1.0, repeat_noise = False):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(
            unet = unet, x=x, t=t, image_embed=image_embed, text_encodings = text_encodings, text_mask = text_mask,
            cond_scale=cond_scale, lowres_cond_img = lowres_cond_img, clip_denoised = clip_denoised, predict_x_start = predict_x_start, learned_variance=learned_variance
        )
        noise = noise_like(x.shape, device, repeat=repeat_noise)
        # no noise when t == 0
        nonzero_mask = (1 - (t==0).float()).reshape(b, *((1,) * (len(x.shape)-1)))
        return model_mean + nonzero_mask * noise * torch.exp(model_log_variance / 2)
    
    @torch.inference_mode()
    def p_sample_loop(self, unet:Unet, shape, image_embed, 
                      predict_x_start = False, clip_denoised = True, learned_variance=False,
                      lowres_cond_img = None, cond_scale = 1.0,
                      text_encodings = None, text_mask = None,):
        device = self.betas.device
    ...
    def p_losses(self, unet:Unet,
        ...
        model_mean, _, model_log_variance = self.p_mean_variance(
            unet, x=x_noisy, t=times, image_embed=image_embed, clip_denoised=clip_denoised, 
            learned_variance=True, model_output=model_output 
        )
        ...
        decoder_nll = -discretized_gaussian_log_likelihood(x_start, means=detached_model_mean,
                                                           log_sacles=0.5*model_log_variance)
    ...
    def sample(
        ...
        assert not (self.condition_on_text_encodings and not exists(text_encodings)), 'must provide text_encodings if condition_on_text_encodings'
        assert not (not self.condition_on_text_encodings and exists(text_encodings)), 'decoder specified not to be conditioned on text, yet it is presented'
        
        img = None
        
        for unet_number, unet, vae, channel, image_size, predict_x_start, learned_variance in tqdm(zip(
    ...
    def forward(
        ...
        assert not (len(self.unets) > 1 and not exists(unet_number)), f'must provide unet_number if there are multiple unets, from range 1 to {len(self.unets)}, if you are training cascading DDPM (multiple unets)'
        ...
        assert h >= target_image_size and w >= target_image_size, f'image must be at least {target_image_size}x{target_image_size}, but get {h}x{w}'
        ...
        if exists(text) and not exists(text_encodings):
            assert exists(self.clip), 'if you are passing in raw text, you must supply `clip` to the decoder on init'
            _, text_encodings, text_mask = self.clip.embed_text(text)
        ...
        return self.p_losses(unet, x_start=image, times = times, image_embed = image_embed, lowres_cond_img = lowres_cond_img, text_encodings = text_encodings, text_mask = text_mask, predict_x_start = predict_x_start, learned_variance = learned_variance)
...
class DALLE2(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.prior_num_samples = piror_num_samples
