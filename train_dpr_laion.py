...
def eval_model(model, device, image_reader, text_reader, 
               start, end, batch_size, loss_type, phase="Validation",):
    model.eval()
    with torch.no_grad():
        total_loss = 0.
        total_samples = 0
...
def report_cosine_sims(diffusion_prior:DiffusionPrior, image_reader, text_reader, train_set_size, val_set_size,
                       NUM_TEST_EMBEDDINGS, device):
    cos = nn.CosineSimilarity(dim=1, eps=1e-6)
    
    tstart = train_set_size + val_set_size # start of test set
    tend = tstart + NUM_TEST_EMBEDDINGS # end of test set
    ...
    for embt, embi in zip(text_reader(batch_size=NUM_TEST_EMBEDDINGS, start=tstart, end=tend),
        ...
        predicted_image_embeddings = diffusion_prior.p_sample_loop((NUM_TEST_EMBEDDINGS, 768), test_text_cond)
        ...
        predicted_unrelated_embeddings = diffusion_prior.p_sample_loop((NUM_TEST_EMBEDDINGS, 768), test_text_shuffled_cond)
...
def train(
    ...
    text_embed_url,
    batch_size,
    train_percent, # percent of data to use for training
    val_percent,   # percent of data to use for validation
    test_percent,  # percent of data to use for testing
    num_epochs,
    ...
    train_set_size = int(num_data_points * train_percent)
    val_set_size = int(num_data_points * val_percent)
    
    for _ in range(epochs):
        diffusion_prior.train()
        
        for emb_images, emb_text in zip(image_reader(batch_size=batch_size, start=0, end=train_set_size), text_reader(batch_size=batch_size, start=0, end=train_set_size)):
            emb_images_tensor = torch.tensor(emb_images[0]).to(device)
            emb_text_tensor = torch.tensor(emb_text[0]).to(device)
            ...
            if (int(time.time() - t) >= save_interval * 60):
                ...
                save_model(
                    save_path=save_path,
                    state_dict=dict(model=diffusion_prior.state_dict(), optimizer=optimizer.state_dict(),
            ...
            if (step % REPORT_METRICS_EVERY == 0):
                diff_cosine_sim = report_cosine_sims(diffusion_prior, 
                                                     image_reader=image_reader, text_reader=text_reader,
                                                     train_set_size=train_set_size, val_set_size=val_set_size,
                                                     NUM_TEST_EMBEDDINGS=NUM_TEST_EMBEDDINGS, device=device)
        ...
        end = train_set_size + val_set_size
        eval_model(model=diffusion_prior, image_reader=image_reader, text_reader=text_reader, start=start, end=end, device=device, loss_type=dp_loss_type, phase='Validation')
        
    # Test run
    test_set_size = int(test_percent * train_set_size)
    start = train_set_size + val_set_size
    end = num_data_points
    eval_model(model=diffusion_prior, image_reader=image_reader, text_reader=text_reader, start=start, end=end, device=device, loss_type=dp_loss_type, phase='Test')
...
def main():
    ...
    print("Wandb logging setup complete.")
    ...
    train(
        ...
        image_embed_url=args.image_embed_url,
        text_embed_url=args.text_embed_url,
